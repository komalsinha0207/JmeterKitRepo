...

{
    ...
    echo "sh PluginsManagerCMD.sh install-for-jmx ${jmx} > /report/injector-plugins-install-${exec_num}.out 2> /report/injector-plugins-install-${exec_num}.err"
    echo "jmeter-server -Dserver.rmi.localport=50000 -Dserver_port=1099 -Djmeter.logfile=/report/jmeter-slave-${exec_num}.log -Jserver.rmi.ssl.disable=true  &"  
    echo "trap 'kill -10 1' EXIT INT TERM"
    echo "java -jar /opt/jmeter/apache-jmeter/lib/jolokia-java-agent.jar start JMeter >> /report/jmeter-injector-jolokia-${exec_num}.out 2>> /report/jmeter-injector-jolokia-${exec_num}.err"
    echo "wait"
...
{ 
    ...
    echo "jmeter ${param_host} ${param_user} ${report_command_line} --logfile /report/${jmx}_${exec_num}.jtl -Djmeter.logfile=/report/master-${jmx}_${exec_num}.log --nongui --testfile ${jmx} -Dserver.rmi.ssl.disable=true --remoteexit --remotestart ${slave_array_final} &"
    echo "trap 'kill -10 1' EXIT INT TERM"
