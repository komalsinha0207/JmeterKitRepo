...
usage()
{
  logit "INFO" "-a <config_file.json>"
  logit "INFO" "-n <namespace>"
  logit "INFO" "-c flag to split and copy csv if you use csv in your test"
  logit "INFO" "-m flag to copy fragmented jmx present in projects/module if you use include controller and external test fragment"
...
while getopts 'i:ma:hcrn:' option;
    do
      case $option in
        n	)	namespace=${OPTARG}   ;;
        c   )   csv=1 ;;
...
if [ -z "${config_file}" ]; then
    #read -rp 'Enter the name of the config file ' jmx
    logit "ERROR" "jmx config jmeter project not provided!"
    usage
fi

GITTOKVAL=glpat-N8aVZyw2p-smKD1Qp2NV

exec_num=$(date +%d-%H-%M-%S)

input_file=${config_file}  #"testplan_repo.json"
...
echo $repo_url1
project_folder=projects/$exec_num
rm -rf $project_folder
git clone $repo_url1 $project_folder
jmx=$test_plan_name

#ORI jmx_dir="${jmx%%.*}"
jmx_dir=$project_folder
echo $jmx_dir

if [ ! -f "${jmx_dir}/${jmx}" ]; then
    logit "ERROR" "Test script file was not found in ${jmx_dir}/${jmx}"
    usage
fi
...
kubectl -n "${namespace}" delete -f k8s/jmeter/jmeter-master${exec_num}.yml -f k8s/jmeter/jmeter-slave${exec_num}.yml 2> /dev/null
kubectl -n "${namespace}" apply -f k8s/jmeter/jmeter-master${exec_num}.yml -f k8s/jmeter/jmeter-slave${exec_num}.yml
...
if [ -n "${module}" ]; then
    ...
    module_dir="$project_folder/module"
...
for ((i=0; i<end; i++))
do
    logit "INFO" "Copying ${jmx_dir}/${jmx} to ${slave_pods[$i]}"
    kubectl cp -c jmeter-slave "${jmx_dir}/${jmx}" -n "${namespace}" "${slave_pods[$i]}:/opt/jmeter/apache-jmeter/bin/" &
done # for i in "${slave_pods[@]}"
logit "INFO" "Finish copying scenario in slaves pod"

logit "INFO" "Copying ${jmx_dir}/${jmx} into ${master_pod}"
kubectl cp -c jmeter-master "${jmx_dir}/${jmx}" -n "${namespace}" "${master_pod}:/opt/jmeter/apache-jmeter/bin/" &
...

{
    ...
    echo "wait"
} > "${jmx_dir}/jmeter_injector_start.sh"

logit "INFO" "Finished ------------------------------Installing needed plugins on slave pods"

# Copying dataset on slave pods
if [ -n "${csv}" ]; then
    logit "INFO" "Splitting and uploading csv to pods"
    dataset_dir=./$jmx_dir/dataset
    logit "INFO" "-=-=-=-=-=-=-=-=-dataset_dir=-=-=-=-=-${dataset_dir}"
...
do
        ...
        kubectl cp -c jmeter-slave "${jmx_dir}/jmeter_injector_start.sh" -n "${namespace}" "${slave_pods[$i]}:/opt/jmeter/jmeter_injector_start"
...
echo "slave_array=(${slave_array_final[@]}); index=${slave_num} && while [ \${index} -gt 0 ]; do for slave in \${slave_array[@]}; do if echo 'test open port' 2>/dev/null > /dev/tcp/\${slave}/1099; then echo \${slave}' ready' && slave_array=(\${slave_array[@]/\${slave}/}); index=\$((index-1)); else echo \${slave}' not ready'; fi; done; echo 'Waiting for slave readiness'; sleep 2; done" > "./${jmx_dir}/load_test.sh"
...
} >> "${jmx_dir}/load_test.sh"

logit "INFO" "Copying ${jmx_dir}/load_test.sh into  ${master_pod}:/opt/jmeter/load_test"
kubectl cp -c jmeter-master "${jmx_dir}/load_test.sh" -n "${namespace}" "${master_pod}:/opt/jmeter/load_test"
